...
And thirdly, we also observe that our new exported one-giant-mesh file has exploded out to 10x the size. This is becuase the [GLTF](../hosting-3d-model/analysis_threejs.md) file format is efficient at grouping unique geometries into one `InstancedMesh`. This means, if we have multiple geometries in the model that have the same mesh, materials, parameters (like steel beams, long pipes, bolts etc), they get saved as one instance in memory. Since we're merging our objects together, we lose the ability to instance our individual meshes and so each vertex needs to be saved to memory, thus exploding the file size.
